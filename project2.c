...
void Timer1_CTC_Init(void) {
    ...
    OCR1A = 977;      // Set compare match value for 1-second delay

    TIMSK |= (1 << OCIE1A); // Enable Timer1 Compare A Interrupt

    // Configure Timer1
    TCCR1A = (1 << FOC1A) | (1 << FOC1B);
    TCCR1B = (1 << WGM12) | (1 << CS12) | (1 << CS10); // CTC Mode, Prescaler = 64
}
...
int main(void) {
    ...
    while (1) {
    	...
    	if (!(PINB & (1 << PB7))) {  // Enter countdown mode if PB7 is pressed
    	    ...
    	    PORTD &= ~(1 << PD5);  // Turn off countdown mode LED
    	    PORTD &= ~(1 << PD0);  // Turn off BUZZER
    	    countdown_mode = 0;
    	    PORTD |= (1 << PD4);  // Set normal mode LED
    	    display_time();  // Update the display in normal mode
    	}
