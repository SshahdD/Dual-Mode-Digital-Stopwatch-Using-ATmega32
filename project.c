...
int main(void) {
    ...
    while (1) {
    	display_time();
    	while (Flag_inc_dec==1) {PORTD &= ~(1 << PD0);  // Turn off BUZZER
    		display_time();
    	...
    	//while(countdown_mode==1)
    	    	     ...
    	    	     if (!(PINB & BUTTON_INC_HOURS)) {
    	    	      HOURS = (HOURS + 1) % 24;
    	    	      while(!(PINB & BUTTON_INC_HOURS)){display_time();};
    	    	    	        }

    	    	    	        // Hours decrement
    	             if (!(PINB & BUTTON_DEC_HOURS)) {
    	    	    	            HOURS = (HOURS == 0) ? 23 : HOURS - 1;
    	    	      while(!(PINB & BUTTON_DEC_HOURS)){display_time();}   	    	    	        }

    	    	    	        // Minutes increment
    	    	    	        if (!(PINB & BUTTON_INC_MINUTES)) {
    	    	    	            MINUTES = (MINUTES + 1) % 60;
    	    	    	            while(!(PINB & BUTTON_INC_MINUTES)){display_time();}
    	    	    	        }

    	    	    	        // Minutes decrement
    	    	    	        if (!(PINB & BUTTON_DEC_MINUTES)) {
    	    	    	            MINUTES = (MINUTES == 0) ? 59 : MINUTES - 1;
    	    	    	            while(!(PINB & BUTTON_DEC_MINUTES)){display_time();}    	    	    	        }

    	    	    	        // Seconds increment
    	    	    	        if (!(PINB & BUTTON_INC_SECONDS)) {
    	    	    	            SECONDS = (SECONDS + 1) % 60;
    	    	    	            while(!(PINB & BUTTON_INC_SECONDS)){display_time();};
    	    	    	        }

    	    	    	        // Seconds decrement
    	    	    	        if (!(PINB & BUTTON_DEC_SECONDS)) {
    	    	    	            SECONDS = (SECONDS == 0) ? 59 : SECONDS - 1;
    	    	    	            while(!(PINB & BUTTON_DEC_SECONDS)){display_time();}    	    	    	        }
